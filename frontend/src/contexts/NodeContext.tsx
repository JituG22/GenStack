...
function createFlowNode(node: Node, onEdit: (id: string) => void, onDelete: (id: string) => void, onClone: (id: string) => void): FlowNode {
  return {
    id: node.id,
    type: 'custom',
...
export function NodeProvider({ children }: { children: ReactNode }) {
  ...
  const loadNodes = useCallback(async (projectId: string) => {
    try {
      ...
      const projectNodes = response.data?.filter((node: Node) => node.projectId === projectId) || [];
      dispatch({ type: 'SET_NODES', payload: projectNodes });
    } catch (error: any) {
      dispatch({ type: 'SET_ERROR', payload: error.message || 'Failed to load nodes' });
    }
  }, []);
  ...
  const saveNodes = useCallback(async () => {
    try {
      ...
      dispatch({ type: 'SET_ERROR', payload: error.message || 'Failed to save nodes' });
    }
  }, []);
